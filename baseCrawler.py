...
class BaseCrawler(object):
    ...
    def __init__(self, court, options, html=None):
        ...
        self.url = {
            "nss": {
                "base": 'http://nssoud.cz',
                "form": 'http://nssoud.cz/main0Col.aspx?cls=JudikaturaExtendedSearch&pageSource=0'
            },
            "us": {
    ...
    def make_soup(path):
        ...
        """
        soup = BeautifulSoup(codecs.open(
            path, encoding="utf-8"), "html.parser")
        return soup
    ...
    def move_files(self):
        if not os.listdir(self.dir_path["result"]):
            ...
            shutil.move(
                join(self.dir_path["working"], self.options.filename), self.dir_path["result"])
    ...
    def extract_data(self, response, html_file):
        """
